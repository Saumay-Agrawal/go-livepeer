...
func (dbo *DBOrchestratorPoolCache) GetOrchestrators(numOrchestrators int) ([]*net.OrchestratorInfo, error) {
	...
	pred := func(info *net.OrchestratorInfo) bool {
		...
		if err := dbo.ticketParamsValidator.ValidateTicketParams(pmTicketParams(info.TicketParams)); err != nil {
			glog.V(common.DEBUG).Infof("unable to validate ticket params - orchestrator=%v recipient=%v err=%v",
				info.GetTranscoder(),
				ethcommon.BytesToAddress(info.GetTicketParams().GetRecipient()),
				err,
			)
			return false
		}
		...
		if maxPrice != nil && price.Cmp(maxPrice) > 0 {
			glog.V(common.DEBUG).Infof("orchestrator's price is too high - orchestrator=%v recipient=%v price=%v maxPrice=%v",
				info.GetTranscoder(),
				ethcommon.BytesToAddress(info.GetTicketParams().GetRecipient()),
				price.FloatString(3),
